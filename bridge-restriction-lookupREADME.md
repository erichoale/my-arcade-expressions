...
Overview
This Arcade expression retrieves bridge weight‑restriction information from a hosted feature layer and identifies which bridges fall within a 100‑meter buffer around the current feature. For each intersecting bridge, the script extracts key restriction attributes and formats them into a readable summary. If no bridges intersect the buffer, the script returns an empty string.

This expression works well for pop‑ups, project reviews, routing evaluations, and any workflow where bridge weight limits near a proposed alignment need to be quickly assessed.

Purpose
Connect to a hosted feature layer using a Portal item ID

Retrieve bridge attributes related to weight restrictions
...
Produce a clean summary suitable for pop‑ups, reports, or analysis tools

How It Works
1. Connect to a Portal
The script initializes a connection to your ArcGIS Online or Enterprise portal.

It loads a feature layer using:

Item ID: <ITEM_ID_REDACTED>

Layer index: 0

Only the required fields are requested:
...
The bridge number is retrieved with a fallback of "N/A"
...
bridgeItemID		Portal item containing bridge data
